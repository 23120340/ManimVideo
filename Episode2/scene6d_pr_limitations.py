...
def verdict_item(label, color, kind, positive=True):
    ...
    if kind == "sensor":
        icon.add(*[
            Dot(LEFT * 0.25 + RIGHT * c * 0.25 + UP * 0.18 + DOWN * r * 0.18, radius=0.035, color=color)
            for r in range(3) for c in range(3)
        ])
    elif kind == "design":
        for x, y in [(-0.35, 0.18), (0, -0.10), (0.35, 0.22)]:
            icon.add(Line(LEFT * 0.45 + RIGHT * (x + 0.45) + UP * y, RIGHT * 0.45 + UP * y, color=color, stroke_width=1.4))
            icon.add(Dot(RIGHT * x + UP * y, radius=0.045, color=YELLOW_3B1B))
    elif kind == "task":
        icon.add(Rectangle(width=0.82, height=0.58, color=color, stroke_width=1.5))
        icon.add(Dot(RIGHT * 0.22 + UP * 0.16, radius=0.055, color=GREEN_3B1B))
        icon.add(VMobject(color=color, stroke_width=2.0).set_points_smoothly([LEFT * 0.28 + DOWN * 0.18, ORIGIN, RIGHT * 0.22 + UP * 0.16]))
    elif kind == "camera":
        icon.add(Rectangle(width=0.78, height=0.55, color=color, stroke_width=1.5))
        icon.add(Circle(radius=0.14, color=color, stroke_width=1.5))
    elif kind == "four":
        icon.add(*[
            Square(side_length=0.22, color=color, fill_color=color, fill_opacity=0.18, stroke_width=1.2).shift(LEFT * 0.14 + RIGHT * c * 0.28 + UP * 0.14 + DOWN * r * 0.28)
            for r in range(2) for c in range(2)
        ])
    else:
        sim = Rectangle(width=0.48, height=0.34, color=BLUE_3B1B, stroke_width=1.4).shift(LEFT * 0.23)
        real = Circle(radius=0.19, color=ORANGE_3B1B, stroke_width=1.4).shift(RIGHT * 0.28)
        icon.add(sim, real, Line(sim.get_right(), real.get_left(), color=GRAY_MID, stroke_width=1.2))
    icon.move_to(frame.get_center() + UP * 0.12)
    mark_color = GREEN_3B1B if positive else RED_BRAIN
    mark = Text("+" if positive else "x", font_size=18, color=mark_color, weight=BOLD)
    mark.move_to(frame.get_top() + DOWN * 0.18)
    text = Text(label, font_size=14, color=GRAY_LIGHT, weight=BOLD, line_spacing=0.9)
    text.scale_to_fit_width(1.18)
    text.move_to(frame.get_bottom() + UP * 0.18)
