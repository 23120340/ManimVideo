...
Re-exports all Episode 1 assets + adds Ep2-specific helpers.

Run: (imported by scene files, not run directly)
"""
"""
...
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Episode1"))
from common import *

_ASSETS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")

# Episode 2 accent color
TEAL_EP2 = "#14B8A6"


# ────────────────────────────────────────────────────────────────
# Helper: simple top-down robot body
# ────────────────────────────────────────────────────────────────
def create_robot_topdown(body_color=BLUE_3B1B, size=0.5):
    """
    Top-down view of a simple robot.
    Returns VGroup(body, heading_arrow)
    """
    body = RoundedRectangle(
        width=size, height=size * 1.2,
        corner_radius=size * 0.15,
        color=body_color, stroke_width=2.5, fill_opacity=0.25,
    )
    heading = Arrow(
        start=body.get_top(),
        end=body.get_top() + UP * size * 0.45,
        color=body_color, buff=0, stroke_width=2,
        max_tip_length_to_length_ratio=0.35,
    )
    return VGroup(body, heading)


# ────────────────────────────────────────────────────────────────
# Helper: pixel grid
# ────────────────────────────────────────────────────────────────
def create_pixel_grid(rows, cols, cell_size=0.18, colors=None, seed=42):
    """
    Creates an n×m grid of colored squares simulating a pixel image.
    Returns VGroup of Square mobjects arranged in a grid.
    """
    import numpy as np
    rng = np.random.default_rng(seed)
    palette = [BLUE_3B1B, GREEN_3B1B, YELLOW_3B1B, RED_BRAIN, PURPLE_3B1B, ORANGE_3B1B, TEAL_EP2]
