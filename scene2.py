...
class Scene2EyeDiversity(Scene):
    def construct(self):
        ...
        intro = vn(
            "Sự đa dạng kinh ngạc của mắt sinh học",
    ...
    def card_1_cat_vs_goat(self):
        header, _, _ = make_card_header(1, 6, "Mèo  vs  Dê")
        ...
        cat_label = vn("Mèo", font_size=26, color=GRAY_LIGHT)
        ...
        goat_label = vn("Dê", font_size=26, color=GRAY_LIGHT)
    ...
    def card_2_eagle_fovea(self):
        header, _, _ = make_card_header(2, 6, "Đại bàng  ·  2 fovea")
        self.play(Write(header), run_time=0.8)
        ...
        human_label = vn("Người", font_size=24, color=GRAY_LIGHT)
        human_label.next_to(human_retina, DOWN, buff=0.5)
        # "1 fovea" - mix VN+EN, dùng vn() cho an toàn
        human_count = vn(
        ...
        eagle_label = vn("Đại bàng", font_size=24, color=GRAY_LIGHT)
        ...
        spiral_label = vn(
            "Quỹ đạo săn mồi: logarithmic spiral",
            font_size=20, color=ORANGE_3B1B, slant=ITALIC,
    ...
    def card_3_scallop(self):
        header, _, _ = make_card_header(3, 6, "Sò điệp  ·  ~200 mắt")
        self.play(Write(header), run_time=0.8)
    ...
    def card_4_cave_fish(self):
        header, _, _ = make_card_header(4, 6, "Cá hang động  ·  tiến hoá xoá mắt")
        self.play(Write(header), run_time=0.8)

        fish_stages = []
        x_positions = [-5, -1.7, 1.7, 5]
        eye_radii = [0.15, 0.10, 0.05, 0.0]
        gen_labels = ["Tổ tiên", "100 thế hệ", "10000 thế hệ", "Hiện tại"]
        ...
        for x, eye_r, gen in zip(x_positions, eye_radii, gen_labels):
            ...
            label = vn(gen, font_size=18, color=GRAY_DIM, slant=ITALIC)
        ...
        why = vn(
            "Vì sao?  →  Mắt tốn năng lượng. Trong hang tối, nó vô ích.",
    ...
    def card_5_tarsier(self):
        header, _, _ = make_card_header(5, 6, "Tarsier  ·  mắt = não")
        self.play(Write(header), run_time=0.8)
        ...
        human_brain_label = vn(
            "não", font_size=20, color=PURPLE_3B1B,
        ...
        human_label = vn(
            "Người", font_size=24, color=GRAY_LIGHT, weight=BOLD,
        ...
        tarsier_brain_label = vn(
            "não", font_size=14, color=PURPLE_3B1B,
        ...
        # mắt ≈ não — có chữ Việt nên dùng vn_markup
        eq = vn_markup(
            'mắt  <span foreground="' + YELLOW_3B1B + '">≈</span>  não',
            font_size=40, color=GRAY_LIGHT,
        ).to_edge(DOWN, buff=1.2)
        caption = vn(
            "cái giá để săn trong đêm",
            font_size=20, color=YELLOW_3B1B, slant=ITALIC,
    ...
    def card_6_butterfly(self):
        header, _, _ = make_card_header(6, 6, "Bướm  ·  hoa văn không dành cho đồng loại")
        self.play(Write(header), run_time=0.8)
        ...
        punch = vn(
            "Hoa văn rực rỡ → dành cho CHIM (kẻ săn nhìn rõ hơn)",
    ...
    def play_recap_grid(self, thumbs):
        recap_title = vn(
            "Sáu loài. Sáu chiến lược thị giác.",
        ...
        closing = vn(
            'Và đây mới là phần dễ thấy — phần cấu trúc bên ngoài.',
            font_size=22, color=GRAY_LIGHT, slant=ITALIC,
