...
class Scene3ZebraTwist(Scene):
    def construct(self):
        ...
        guess_a = vn(
            "A. Ngụy trang khỏi sư tử",
            font_size=22, color=GRAY_LIGHT,
        ).next_to(lion_icon, RIGHT, buff=0.4)
        guess_b = vn(
            "B. Trông như đàn lớn hơn",
            font_size=22, color=GRAY_LIGHT,
        ).next_to(zebra_icon, RIGHT, buff=0.4)

        # Thu nhỏ ngựa vằn lên trên để nhường không gian
        self.play(
            zebra.animate.scale(0.55).move_to(UP * 1.5 + LEFT * 4.5),
            question.animate.scale(0.65).next_to(zebra, RIGHT, buff=0.5).set_color(GRAY_LIGHT),
            run_time=1.2,
        )
        ...
        # Punchline
        punch = vn(
            "Trừ một chi tiết: sư tử KHÔNG nhìn ra sọc từ khoảng cách săn.",
        ...
        slip_label = vn(
            "Muỗi không hạ cánh được — sọc gây nhiễu mắt kép",
        ...
        stat_text = vn_markup(
            'Trang trại sơn sọc bò  →  '
            f'<span foreground="{GREEN_3B1B}"><b>giảm bệnh do muỗi truyền</b></span>',
            font_size=22, color=GRAY_LIGHT,
        ...
        answer_vn = vn(
            "(Câu trả lời: muỗi)",
            font_size=26, color=GRAY_LIGHT, slant=ITALIC,
        ...
        moral_l1 = vn(
            "Khi đoán  «trí tuệ thiết kế»  theo trực giác con người,",
        ...
        moral_l3 = vn(
            "Cơ thể không tiến hoá để  CHÚNG TA  thấy đẹp.",
            font_size=24, color=GRAY_LIGHT, slant=ITALIC,
        )
        moral_l4 = vn(
            "Nó tiến hoá để giải bài toán mà nó đang đối mặt.",
