...
class Scene4MathFormulation(Scene):
    def construct(self):
        ...
        # Intro
        intro = vn(
            "Gói gọn bài toán bằng toán học",
        ...
        ps_caption = vn(
            'mỗi điểm = một số  ("tỉ lệ sống sót")',
            font_size=18, color=GRAY_LIGHT, slant=ITALIC,
        ...
        eq.move_to(UP * 2.7)
        # Subscript θ dưới argmax — vẽ thủ công nhỏ kế bên
        ...
        # Vấn đề: U không có công thức đóng
            ...
            "Vấn đề: U(θ) không có công thức đóng.",
            font_size=24, color=RED_BRAIN, weight=BOLD,
        ).to_edge(DOWN, buff=0.4)
        catch_2 = vn(
            "Phải mô phỏng cơ thể trong môi trường để biết U.",
        ...
        # Caption nhỏ ở dưới
        pipe_cap = vn(
            "Mỗi lần thử một thiết kế θ mới = chạy lại toàn bộ pipeline.",
        ...
        # ============================================================
        branch_title = vn(
            "Hai trường phái giải bài toán này:",
        ...
        phys_caption = vn(
            "biết vật lý  →  viết mô hình  →  tối ưu",
            font_size=18, color=GRAY_LIGHT, slant=ITALIC,
        ...
        # Input/output icons
        input_icon = vn("mắt", font_size=16, color=GRAY_LIGHT)
        input_icon.next_to(nn_grp, LEFT, buff=0.25)
        output_icon = vn("hành\nđộng", font_size=16, color=GRAY_LIGHT, line_spacing=0.8)
        output_icon.next_to(nn_grp, RIGHT, buff=0.25)

        learn_caption = vn(
            "không có công thức  →  học từ dữ liệu",
            font_size=18, color=GRAY_LIGHT, slant=ITALIC,
        ...
        bridge = vn_markup(
            'Cơ thể (cứng)  →  <span foreground="' + BLUE_3B1B + '">trái</span>  ·  '
            'Mắt → hành vi  →  <span foreground="' + PURPLE_3B1B + '">phải</span>',
