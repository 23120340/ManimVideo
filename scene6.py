...
class Scene6Cliffhanger(Scene):
    def construct(self):
        ...
        recap_title = vn(
            "Quay lại từ đầu: hai tập tham số",
        ...
        components = [
            ("shape",          "hình dạng cơ thể"),
            ("eye position",   "vị trí mắt"),
            ("eye resolution", "độ phân giải mắt"),
            ("field of view",  "trường nhìn"),
            ("sensor count",   "số lượng cảm biến"),
        ]

        comp_group = VGroup()
        for i, (en, vn_label) in enumerate(components):
            y = 2.2 - i * 0.95
            ...
            en_text = Text(
                en, font_size=18, color=BLUE_3B1B, weight=BOLD,
            ).move_to(box.get_left() + RIGHT * 1.2)
            vn_text = vn(
                vn_label, font_size=18, color=GRAY_LIGHT, slant=ITALIC,
            ).move_to(box.get_right() + LEFT * 1.0)
            comp_group.add(VGroup(box, en_text, vn_text))
        ...
        # ============================================================
        section_title = vn(
            "Tự nhiên không dùng camera độ phân giải cao.",
        ...
        question_below = vn(
            "Liệu nó còn  thấy  được mục tiêu không?",
        ...
        # ============================================================
        self.play(
            FadeOut(VGroup(section_title, question_below)),
            view.animate.scale(0.7).shift(LEFT * 2.5),
            res_label.animate.scale(0.7).shift(LEFT * 2.5),
            run_time=1.0,
        )
        ...
        cliff_l3 = vn(
            "có thể điều hướng trong nhà không?",
        ...
        # ============================================================
        self.play(
            FadeOut(VGroup(view, res_label, cliff_group)),
            run_time=1.2,
        )

        ep2_title = Text(
            "Tập 2", font_size=32, color=GRAY_DIM, slant=ITALIC,
        )
        ep2_main = vn(
            "Thấy thế giới bằng 4 pixel",
